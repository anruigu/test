  ...
  **/
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          int min_dist_idx = x_points.size() - 1; //idx
          double min_dist = std::numeric_limits<double>::max();
          for (int j = 0; j < x_points.size(); j++){
            min_dist = std::hypot(x_points[min_dist_idx] - x_position, y_points[min_dist_idx] - y_position); // euclidean distance 
            double dist = std::hypot(x_points[j] - x_position, y_points[j] - y_position);
            if (dist < min_dist){
              min_dist = dist;
              min_dist_idx = j;
            }
          }
          //to steer, we need to turn from current position to the closest point in planned traj 
         double planned_yaw = angle_between_points(x_position, y_position, x_points[min_dist_idx], y_points[min_dist_idx]);
          //Error = current yaw - planned yaw
          error_steer = planned_yaw - yaw;
          ...
          **/
          // Get the difference between the planned and actual velocity to use as metric for this timestep 
